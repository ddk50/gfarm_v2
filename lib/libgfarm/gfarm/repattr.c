/*
 * $Id: repattr.c 8435 2013-07-29 03:38:12Z takuya-i $
 */
#include <limits.h>
#include <stddef.h>
#include <ctype.h>
...
allocate_repattr(const char *fsngroupname, size_t n)
{
	...
	if (ret == NULL) {
		gflog_debug(GFARM_MSG_1003889, "allocate_repattr(): %s",
			gfarm_error_string(GFARM_ERR_NO_MEMORY));
		goto done;
	}
	tmp_fsng = strdup(fsngroupname);
	if (tmp_fsng == NULL) {
		gflog_debug(GFARM_MSG_1003890, "allocate_repattr(): %s",
			gfarm_error_string(GFARM_ERR_NO_MEMORY));
		goto done;
	}
...
gfarm_repattr_parse(const char *s, gfarm_repattr_t **retp, size_t *n_retp)
{
	...
	if (buf == NULL) {
		...
		gflog_debug(GFARM_MSG_1003891, "gfarm_repattr_parse(): %s",
	...
	if (tokens == NULL) {
		gflog_debug(GFARM_MSG_1003892, "gfarm_repattr_parse:() %s",
			gfarm_error_string(GFARM_ERR_NO_MEMORY));
		goto done;
	}
	...
	if (reps == NULL) {
		e = GFARM_ERR_NO_MEMORY;
		gflog_debug(GFARM_MSG_1003893, "gfarm_repattr_parse:() %s",
			gfarm_error_string(e));
		goto done;
	}
